...
def preprocess_2(y, Aw, Aw2, Ab, N, T):
    #label
    label = np.concatenate(y)
    #persist
    persist = np.concatenate(y[:(T-1)])
...
class ClsnaModel_2(torch.nn.Module):
    def __init__(self,device,N,T,ar_pair,Aw,Aw2,Ab,D=2):
        """
        A CLSNA model for training on dynamic networks using PyTorch (with different within-group attraction paraemters Aw, Aw2).
    ...
    def loss(self, device,label,persist,sample_edge,T_index,ss=1,tt=1):
        ...
        #T>1 attractor    
        att_w = gw*torch.sparse.mm(self.Aw,self.z[:(self.T-1)*self.N])
        att_w2 = gw2*torch.sparse.mm(self.Aw2,self.z[:(self.T-1)*self.N])
        att_b = gb*torch.sparse.mm(self.Ab,self.z[:(self.T-1)*self.N])
        _p3 = self.z[self.ar_pair[:,1]]-self.z[self.ar_pair[:,0]]-(att_w+att_b+att_w2)
        p3 = -_p3**2/2/tau2-logtau2/2
        #combine p2 p3
        pt = torch.cat((p2,p3),dim = 0)[T_index]
        #adjust*T_index/edge_index = 2/(N-1)
        adjust = 2*sample_edge.size(0)/T_index.size(0)/(self.N-1)
        return -(p1+adjust*torch.sum(pt)-(alpha-0)**2/2/100-(delta-0)**2/2/100-(gw-0.5)**2/2/100-(gw2-0.5)**2/2/100-(gb+0.5)**2/2/100)
#         return -(p1+adjust*torch.sum(pt))