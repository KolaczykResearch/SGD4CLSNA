...
def preprocess_congress(y, Aw, Ab, n_nodes, persist):
    #label
    label = np.concatenate(y)
    #persist
    persist = np.concatenate([np.zeros(y[0].shape[0]),persist])
    ...
    sparse_Ab = sparse.block_diag(sparse_Ab)
    #combination_N
    combination_N = torch.combinations(torch.arange(n_nodes[0]))
    start = n_nodes[0]
    for nn in n_nodes[1:]:
        _combination_N = torch.combinations(torch.arange(nn))
        combination_N = torch.cat((combination_N, _combination_N+start), dim = 0)
        start = start+nn
    return from_numpy(label), from_numpy(persist), convert(sparse_Aw), convert(sparse_Ab), combination_N

def preprocess_congress_2(y, Aw, Aw2, Ab, n_nodes, persist):
    #label
    label = np.concatenate(y)
    #persist
    persist = np.concatenate([np.zeros(y[0].shape[0]),persist])
    ...
    sparse_Aw2 = []
    for aw2 in Aw2:
        sparse_Aw2.append(sparse.csr_matrix(aw2))
    sparse_Aw2 = sparse.block_diag(sparse_Aw2)
    #Ab
    sparse_Ab = []
    for ab in Ab:
        sparse_Ab.append(sparse.csr_matrix(ab))
    ...
    for nn in n_nodes[1:]:
        ...
        combination_N = torch.cat((combination_N, _combination_N+start), dim = 0)
        start = start+nn
    return from_numpy(label), from_numpy(persist), convert(sparse_Aw), convert(sparse_Aw2), convert(sparse_Ab), combination_N
...
class ClsnaModelCongress(torch.nn.Module):
    ...
    """ 
    def __init__(self,device,n_nodes,T,ar_pair,Aw,Aw2,Ab,new_at_t,member_at_t,D=2):
        super().__init__()
        self.D = D
        self.n_nodes = n_nodes
        self.T = T
        self.z = Parameter(torch.randn((np.sum(n_nodes),D)), requires_grad = True)
        self.para = Parameter(torch.zeros((4, 2)), requires_grad = True)
        ...
        self.Aw2 = Aw2.to(device).coalesce()
        self.Ab = Ab.to(device).coalesce()
        self.Aw_dim = Aw.shape[0]
        self.new_at_t = new_at_t
        self.member_at_t = member_at_t

    def forward(self):
        return self.z, self.para
    
    def calculate_auc(self, device, label, persist, sample_edge):
        ...
        # Convert log probabilities to actual probabilities for AUC calculation
        probabilities = torch.sigmoid(eta).detach().cpu().numpy()
    ...
    def loss(self,device,label,persist,sample_edge,T_index,ss=1,tt=1,pp=1):
        ...
        #T=1 attractor
        p2 = -self.z[:self.n_nodes[0]]**2/2/sigma2-logsigma2/2
        #T>1 and get reelected attractor
        att_w = gw*torch.sparse.mm(self.Aw,self.z[:self.Aw_dim])
        att_w2 = gw2*torch.sparse.mm(self.Aw2,self.z[:self.Aw_dim])
        att_b = gb*torch.sparse.mm(self.Ab,self.z[:self.Aw_dim])
        _p3 = self.z[self.ar_pair[:,1]]-self.z[self.ar_pair[:,0]]-(att_w+att_b+att_w2)[self.ar_pair[:,0]]
        
        p3 = -_p3**2/2/tau2-logtau2/2
        #T>1 and get newly elected attractor
        for time in range(1,self.T):
            for party in range(2):
                member_indices = self.member_at_t[time-1][party]
                party_mean = torch.mean(self.z[member_indices],dim=0)
                absent_indices = self.new_at_t[time][party]
                _p4 = self.z[absent_indices]-party_mean
